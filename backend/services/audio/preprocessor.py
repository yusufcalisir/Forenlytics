...
MIN_SPEECH_SEC: float = 1.5      # Reject files with < 1.5s of detected speech
VAD_FRAME_MS: int = 20           # Frame length for energy VAD (20ms is standard)
VAD_ENERGY_PERCENTILE: float = 5.0  # Frames above P5 of energy are "speech"
VAD_ENERGY_MARGIN_DB: float = 12.0  # Extra dB above floor before a frame is "speech"
...
class AudioPreprocessor:
    ...
    def _apply_vad(self, y: np.ndarray, steps: List[str]) -> np.ndarray:
        ...
        Splits audio into 20ms frames, computes per-frame RMS energy,
        keeps frames whose energy exceeds a threshold derived from the signal's
        noise floor. Concatenates active frames into a single output array.

        This removes leading/trailing silence AND long mid-utterance silent gaps.
        """
        ...
        # Threshold: noise floor estimate (low percentile of energy) + margin
        ...
        # If VAD kills > 95% of signal, the file is likely all noise/silence
        ...
        # Concatenate active frames
        active_frames = frames[active_mask]
        y_vad = active_frames.flatten()

        # Include any remaining samples (last partial frame) if the full array is active
        remainder = y[num_frames * frame_len :]
        if len(remainder) > 0 and active_mask[-1]:
            y_vad = np.concatenate([y_vad, remainder])

        steps.append(
            f"VAD: {active_mask.sum()}/{num_frames} frames active "
            f"({active_ratio*100:.0f}% of audio retained, "
            f"threshold: noise_floor {noise_floor_db:.1f}dB + {VAD_ENERGY_MARGIN_DB}dB margin)"
        )
        return y_vad
